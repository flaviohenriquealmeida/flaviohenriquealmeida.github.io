...
O exemplo abaixo, que estouraria a *call stack* em uma *engine* JavasScript **com** TCO rodará indefinidamente sem provocar o estouro por uma engine com suporte a TCO:
...
E nossa função `factorial`? Ela não se coaduna com a exigência da TCO, pois seu retorno é `n * factorial(n - 1)` e não a chamada de uma função apenas. 
